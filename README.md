...
This is a hobby project for logging hikes and travels.
Web page of a project is at [hikelog.org](https://hikelog.org) (Yep, still nothing there yet), 
real application is at [zuzkajde.cz](https://zuzkajde.cz).


Still work in progress, but it is working OKish.
Especially UI on phones needs some polishing.

This repository is ment for people with programming skills... or some AI help now :)

I'm aiming to make it public as a service eventually
(free without AI/photos/videos/audio, paid with those, because it is not cheap), 
but I want to keep it as OSS (unless someone wants to buy it for a fortune :D ) .

Admin UI is quite crude – a lot of unnecessary internal info is exposed and some is missing...  
and needs a lot of polishing to be better usable on phones – it is usable though.
...
* Postgres database
* Firebase storage as S3-compatible media storage (will probably change to something else later, but it works for now)
* Google API for speech-to-text transcription and admin login
* AI audio enhancement with OpenAI
* Google Analytics for tracking
* Mapy.com API for maps
* WebPushR for push notifications

Contact me if you are brave enough to try it out, have some questions or suggestions.

The rest is AI-generated - but if you stick to it, you can make it running.
